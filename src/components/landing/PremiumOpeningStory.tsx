...
export function PremiumOpeningStory(){
 ...
 return <section ref={ref} className="relative h-[620svh] bg-[#FFFDFB]">
  <div className="sticky top-0 h-[100svh] overflow-hidden">
   ...
   <div className="absolute inset-0 flex flex-col px-5 pb-5 pt-5 will-change-transform" style={{opacity:1-heroOut,transform:`translate3d(0,${-heroOut*70}px,0) scale(${1-heroOut*.055})`,pointerEvents:heroOut>.75?"none":"auto"}}>
    <p className="text-[10px] font-bold uppercase leading-5 tracking-[.27em] text-[#A078B0]">Sri Lankan matchmaking<br/>for a brighter tomorrow</p>
    <h1 className="mt-3 font-serif text-[2.7rem] font-semibold leading-[.98] text-[#2A1845]">Find Your<br/><span className="text-[#C026D3]">Perfect Match ♡</span></h1>
    <p className="mt-3 max-w-sm text-[12px] leading-5 text-[#65546F]">A trusted matrimony platform for the Sri Lankan community in the UK, Canada, Australia and beyond.</p>
    <div className="mt-4 flex justify-between px-2">{[[Users,"Verified\nProfiles"],[Shield,"Safe & Secure"],[Heart,"Find\nCompatibility"]].map(([Icon,label]:any,i)=><div key={i} className="w-24 text-center" style={{opacity:phase(p,.005+i*.012,.06+i*.012),transform:`translateY(${(1-phase(p,.005+i*.012,.06+i*.012))*12}px)`}}><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#F3E8FF] text-[#7C3AED]"><Icon className="h-5 w-5"/></span><span className="mt-2 block whitespace-pre-line text-[10px] font-semibold leading-3 text-[#4C3A5C]">{label}</span></div>)}</div>
    <div className="mt-4 grid gap-2"><Link href="/signup" className="flex h-11 items-center justify-center rounded-full bg-[#7C3AED] text-sm font-semibold text-white shadow-lg">Create Your Profile <ArrowRight className="ml-1 h-4 w-4"/></Link><Link href="/discover" className="flex h-11 items-center justify-center rounded-full border border-violet-200 bg-white text-sm font-semibold text-violet-700">Explore Matches</Link></div>
   ...
   <div className="absolute inset-0 flex items-center justify-center px-5" style={{opacity:tagline*(1-phase(p,.32,.38)),transform:`translate3d(0,${(1-tagline)*65-phase(p,.32,.38)*-35}px,0)`}}>
    <div className="relative h-[82svh] w-full max-w-[390px]">
     ...
     <div className="absolute bottom-0 right-0 h-[62svh] w-[88%] overflow-hidden rounded-t-[7rem] rounded-b-[2.4rem] shadow-2xl" style={{transform:`translateY(${(1-tagline)*50}px) scale(${.94+tagline*.06})`}}><Image src="/images/values/culture.jpg?v=3" alt="" fill sizes="390px" className="object-cover"/></div>
    </div>
   </div>
   ...
   <div className="absolute inset-0 flex flex-col items-center justify-center px-5" style={{opacity:connect,transform:`translateY(${(1-connect)*55}px)`}}>
    <div className="text-center"><h2 className="font-serif text-[2.25rem] font-semibold leading-[1.02] text-[#2A1845]">A Meaningful<br/><span className="text-[#C026D3]">Connection Begins</span></h2><p className="mt-3 text-sm text-[#6B5A78]">When shared values bring two people together.</p></div>
    <div className="relative mt-5 h-[430px] w-full max-w-[360px]">
    ...
    <div className="grid w-full max-w-[280px] gap-2 text-xs font-medium text-[#5C4A66]">{["Shared values","Similar goals","Cultural alignment","Real conversations"].map((x,i)=><div key={x} className="flex items-center gap-2" style={{opacity:phase(connect,.35+i*.1,.6+i*.1),transform:`translateX(${(1-phase(connect,.35+i*.1,.6+i*.1))*18}px)`}}><span className="flex h-5 w-5 items-center justify-center rounded-full bg-violet-100 text-[#7C3AED]">✓</span>{x}</div>)}</div>
   </div>
  </div>
 </section>
}