...
as $$
declare v_user uuid := auth.uid(); v_period date := date_trunc('month', now())::date; v_used integer;
begin
  if v_user is null then raise exception 'not authenticated'; end if;
  insert into public.subscription_usage(user_id,feature_key,period_start,used_count)
  values(v_user,p_feature_key,v_period,0)
