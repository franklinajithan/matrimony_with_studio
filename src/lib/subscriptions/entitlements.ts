...
export function effectivePlanCode(subscription?: SubscriptionState | null): PlanCode {
  if (!subscription?.planCode || !PAID_STATES.has(subscription.status ?? '')) return 'free';
  if (subscription.currentPeriodEnd && new Date(subscription.currentPeriodEnd).getTime() <= Date.now()) return 'free';
...
export function hasSubscriptionEntitlement(subscription: SubscriptionState | null | undefined, key: EntitlementKey) {
  return hasEntitlement(effectivePlanCode(subscription), key);
}
