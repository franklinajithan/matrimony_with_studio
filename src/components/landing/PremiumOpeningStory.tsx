...
const openingCopy = {
 en:["{oc[0]}","{oc[1]}","{oc[2]}","{oc[3]}","{oc[4]}","Discover","{oc[6]}","{oc[7]}","Shared values","Life goals","Culture","Family","{oc[12]}","A Meaningful","{oc[14]}","{oc[15]}","Similar goals","Cultural alignment","Real conversations"],
...
export function PremiumOpeningStory(){
 ...
 return <section className="bg-[#FFFDFB]">
  ...
  <article className="homepage-story-card grid items-center gap-8 overflow-hidden px-5 py-8 lg:grid-cols-[.82fr_1.18fr] lg:gap-16 lg:px-[max(6vw,72px)] xl:mx-auto xl:max-w-[1500px]">
   <div className="story-copy text-center lg:text-left"><p className="mb-4 text-[10px] font-bold uppercase tracking-[.28em] text-[#A078B0] lg:text-xs">Built around real compatibility</p><h2 className="font-serif text-[2.25rem] font-semibold leading-[1.06] text-[#2A1845] lg:text-[4.65rem]">{oc[13]}<br/><span className="text-[#C026D3]">Connection Begins</span></h2><p className="mt-4 text-sm leading-6 text-[#6B5A78] lg:max-w-lg lg:text-lg lg:leading-8">When shared values bring two people together, a conversation can become something more.</p><div className="mt-7 grid max-w-[430px] grid-cols-2 gap-3 text-sm font-medium text-[#5C4A66]">{[oc[8],oc[16],oc[17],oc[18]].map((x,i)=><div key={x} className="story-item flex items-center gap-2 rounded-2xl border border-violet-100 bg-white px-3 py-3 shadow-sm" style={{animationDelay:`${i*70}ms`}}><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-violet-100 text-[#7C3AED]">✓</span>{x}</div>)}</div></div>
