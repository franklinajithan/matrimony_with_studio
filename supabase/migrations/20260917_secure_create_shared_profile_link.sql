...
begin
  ...
  if not exists (select 1 from public.profiles p where p.id = p_profile_id and coalesce(p.is_published, false) = true) then
    raise exception 'Profile not found or unavailable';
  end if;
  if (select count(*) from public.profile_share_links l where l.owner_id = v_owner and l.created_at >= now() - interval '24 hours') >= 10 then
    raise exception 'Share limit reached. Try again later.';
  end if;
  if (select count(*) from public.profile_share_links l where l.owner_id = v_owner and l.revoked_at is null and l.expires_at > now()) >= 3 then
