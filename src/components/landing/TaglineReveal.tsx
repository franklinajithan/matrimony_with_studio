...
export function TaglineReveal() {
  ...
  return <section ref={ref} className="relative flex min-h-[100svh] items-center overflow-hidden bg-[#FFFDFB] px-5 py-10">
    ...
    <div className="relative mx-auto w-full max-w-[390px]">
      <div className="relative z-10 mb-[-1.6rem] ml-2">
        <p className="font-script text-[3rem] leading-[.9] text-[#C026D3] transition-all duration-1000 sm:text-6xl" style={{opacity:on?1:0,transform:on?"translate3d(0,0,0)":"translate3d(-24px,12px,0)"}}>Good people<span className="block ml-9">Brighter futures</span></p>
        <svg viewBox="0 0 330 100" className="mt-1 h-20 w-full overflow-visible" fill="none" aria-hidden="true"><path d="M10 42 C70 88 116 5 177 45 C222 76 264 28 318 55" stroke="#D946EF" strokeWidth="2" strokeLinecap="round" pathLength="1" style={{strokeDasharray:1,strokeDashoffset:on?0:1,transition:"stroke-dashoffset 1400ms cubic-bezier(.2,.8,.2,1) 250ms"}}/><path d="M300 51 C294 43 282 50 300 66 C318 50 306 43 300 51Z" fill="#D946EF"/></svg>
      </div>
      <div className="relative ml-auto h-[54svh] min-h-[380px] w-[86%] overflow-hidden rounded-t-[7rem] rounded-b-[2.5rem] shadow-[0_24px_65px_rgba(76,29,149,.16)] transition-all duration-1000" style={{opacity:on?1:0,transform:on?"translate3d(0,0,0) scale(1)":"translate3d(0,45px,0) scale(.96)"}}>
