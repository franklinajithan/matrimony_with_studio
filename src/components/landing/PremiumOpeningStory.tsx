...
export function PremiumOpeningStory(){
 ...
 return <section ref={ref} className="relative h-[500svh] bg-[#FFFDFB] [scroll-snap-align:start] [scroll-snap-stop:always]">
  <div className="sticky top-0 h-[100svh] overflow-hidden">
   ...
   <div className="absolute inset-0 flex flex-col px-5 pb-5 pt-5 will-change-transform lg:grid lg:grid-cols-[minmax(420px,0.85fr)_minmax(520px,1.15fr)] lg:items-center lg:gap-14 lg:px-[7vw] lg:py-10" style={{opacity:1-heroOut,transform:`translate3d(0,${-heroOut*70}px,0) scale(${1-heroOut*.055})`,pointerEvents:heroOut>.75?"none":"auto"}}>
    <div className="lg:max-w-[560px]"><p className="text-[10px] font-bold uppercase leading-5 tracking-[.27em] text-[#A078B0] lg:text-xs">Sri Lankan matchmaking<br/>for a brighter tomorrow</p>
    <h1 className="mt-3 font-serif text-[2.7rem] font-semibold leading-[.98] lg:mt-5 lg:text-[4.8rem] text-[#2A1845]">Find Your<br/><span className="text-[#C026D3]">Perfect Match ♡</span></h1>
   ...
   <div className="absolute inset-0 flex items-center justify-center px-5" style={{opacity:tagline*(1-phase(p,.36,.41)),transform:`translate3d(0,${(1-tagline)*48-phase(p,.36,.41)*-22}px,0)`}}>
    <div className="relative h-[82svh] w-full max-w-[390px] lg:h-[72vh] lg:max-w-5xl">
     <div className="absolute left-2 top-12 z-10 font-script text-[3.1rem] lg:left-8 lg:top-1/3 lg:text-[5.2rem] leading-[.9] text-[#C026D3]">Good people<span className="ml-8 block">Brighter futures</span></div>
     ...
     <div className="absolute bottom-0 right-0 h-[62svh] w-[88%] lg:h-[70vh] lg:w-[48%] overflow-hidden rounded-t-[7rem] rounded-b-[2.4rem] shadow-2xl" style={{transform:`translateY(${(1-tagline)*50}px) scale(${.94+tagline*.06})`}}><Image src="/images/values/culture.jpg?v=3" alt="" fill sizes="390px" className="object-cover"/></div>
    </div>
   </div>

   <div className="absolute inset-0 flex flex-col items-center justify-center px-4 lg:px-[7vw]" style={{opacity:cards*(1-phase(p,.61,.66)),transform:`translateY(${(1-cards)*55}px)`}}>
    <div className="text-center lg:text-left"><h2 className="font-serif text-[2.35rem] font-semibold leading-[1.02] text-[#2A1845]">Discover<br/><span className="text-[#C026D3]">Compatible Matches</span></h2><p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-[#6B5A78]">Meet people who share your values, culture and life goals.</p></div>
    <div className="relative mt-8 h-[390px] w-full max-w-[390px] lg:mt-10 lg:h-[460px] lg:max-w-[760px]">
     {profiles.map((x,i)=>{const side=i-1;const xPos=side*(typeof window!=="undefined"&&window.innerWidth>=1024?245:145);const y=Math.abs(side)*38;const rot=side*7;const sc=i===1?1:.82;return <div key={x.name} className="absolute left-1/2 top-0 w-[220px] overflow-hidden rounded-[2rem] border-[5px] border-white bg-white shadow-[0_24px_60px_rgba(76,29,149,.16)] will-change-transform" style={{zIndex:i===1?3:1,opacity:cards,transform:`translate3d(calc(-50% + ${xPos*cards}px),${(1-cards)*90+y}px,0) rotate(${rot*cards}deg) scale(${.8+(sc-.8)*cards})`}}><div className="relative h-[275px]"><Image src={x.src} alt="" fill sizes="220px" className="object-cover"/>{i===1&&<span className="absolute bottom-3 right-3 rounded-full bg-white px-3 py-1 text-xs font-bold text-[#C026D3] shadow">92% Match</span>}</div><div className="p-4"><p className="font-serif text-xl font-semibold text-[#2A1845]">{x.name}</p><p className="mt-1 text-xs text-[#75647F]">{x.place}</p></div></div>})}
    </div>
   </div>

   <div className="absolute inset-0 flex flex-col items-center justify-center px-5 lg:grid lg:grid-cols-[0.8fr_1.2fr] lg:gap-16 lg:px-[10vw]" style={{opacity:connect,transform:`translateY(${(1-connect)*55}px)`}}>
    <div className="text-center"><h2 className="font-serif text-[2.25rem] lg:text-[4.2rem] font-semibold leading-[1.02] text-[#2A1845]">A Meaningful<br/><span className="text-[#C026D3]">Connection Begins</span></h2><p className="mt-3 text-sm lg:max-w-md lg:text-base lg:leading-7 text-[#6B5A78]">When shared values bring two people together.</p></div><div className="lg:relative">
